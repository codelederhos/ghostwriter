...
async function runPipelineForLanguage(data, language, preview, opts = {}) {
  ...
  try {
    ...
    return { postId, language, title: article.title, slug: article.slug, status: reviewMode ? "draft_review" : (preview ? "draft" : "published"), durationMs: totalMs, _article: article };
  } catch (err) {
    if (postId) {
