...
async function loadTenant(tenantId) {
  ...
  // System-Config: konfigurierte Modell-Präferenzen laden
  const { rows: sysRows } = await query("SELECT key, value FROM system_config WHERE key IN ('recommended_models', 'image_models')");
  const sysConfig = {};
  ...
  if (decrypted.billing_mode === "platform") {
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      // Konfiguriertes Bild-Modell aus Settings (default: gpt-image-1)
      decrypted.image_model = sysConfig.image_models?.openai?.model || "gpt-image-1";
    }
  } else if (!decrypted.image_model) {
    // Own-Key-Mode: Modell aus system_config falls tenant keins konfiguriert hat
    decrypted.image_model = sysConfig.image_models?.openai?.model || "gpt-image-1";
  }
