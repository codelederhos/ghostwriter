...
async function runPipelineForLanguage(data, language, preview) {
  ...
  try {
    ...
    if (settings.research_enabled) {
      ...
      if (researchFacts) {
        await logStep(tenantId, null, "research", "success", `${researchFacts.length} Zeichen Recherche`, Date.now() - start);
      }
    }
    ...
    try {
      ...
      await logStep(tenantId, null, "image", "success", `Blog: ${imageResult.url}, GBP: ${imageResult.url2}`, Date.now() - start);
    } catch (err) {
