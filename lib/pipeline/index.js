...
async function runPipelineForLanguage(data, language, preview, opts = {}) {
  ...
  try {
    ...
    // Step 4b: CHART (inline Chart.js, kein QuickChart PNG mehr)
    if (article.chart_config) {
      start = Date.now();
      try {
        const configJson = JSON.stringify(article.chart_config);
        const chartHtml = `<div class="gw-chart"><script type="application/json" class="gw-chart-config">${configJson}<\/script></div>`;
        ...
        await logStep(tenantId, null, "chart", "success", "Chart-HTML eingebettet", Date.now() - start);
      } catch (err) {
