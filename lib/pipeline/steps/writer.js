...
POSITION 2 (nach ~50% Inhalt, mittiger Artikel-Bereich): AUTHORITY-CALLOUT mit Tenant-Anekdote + Soft-CTA
  ...
  Beispiel:
  <aside class="callout callout--cta"><p><strong>Aus der Arbeit von ${companyName}:</strong> Eine frühe Klärung der fachlichen und organisatorischen Fragen schafft eine belastbare Entscheidungsgrundlage. Beschreibe ausschließlich einen aus dem Firmenprofil belegbaren Nutzen, ohne Zahlen oder angebliche Kundenerfahrungen zu erfinden.</p><p><a href="${ctaUrl}" class="callout__link">Unverbindlich anfragen →</a></p></aside>
...
Statt plakativem Eigenlob beschreibst du nur belegbare Arbeitsweisen, Leistungen
und Vorteile aus dem Firmenprofil. Keine angeblichen Beobachtungen, Gespräche,
Kundenfälle, Häufigkeiten, Markttrends oder Erfahrungszahlen erfinden.
Formulierungen beginnen zum Beispiel mit "Bei ${companyName} ist der Ablauf so
aufgebaut, dass …" oder "${companyName} verbindet [belegte Leistung A] mit
[belegter Leistung B]". Mindestens ein solcher belegbarer Praxisbezug darf im
Fließtext und im Callout Position 2 stehen.
...
Antworte NUR im JSON-Format:
{
  ...
  "primary_keyword": "...",
  "social_text": "...",
  "image_prompt_1": "English, one coherent editorial scene (max 220 chars): a specific real candid moment that directly explains the article topic for ${profile?.industry || "the business"} in ${profile?.region || "Germany"}. Wide composition, Central European setting, natural daylight. No staged office stock, no readable screens, no floating UI, no cyber clichés.",
  "image_format_1": "landscape",
  "image_prompt_2": "English, a clearly different but directly relevant editorial scene (max 220 chars): a concrete close-up, physical process or second real moment that adds information. Wide composition, natural daylight. No generic hands-at-laptop, no readable documents/screens, no floating UI, no cyber clichés.",
  "image_format_2": "landscape",
  "chart_config": null
}

DIAGRAMM-REGEL (chart_config):
- Ein Diagramm ist NUR erlaubt, wenn jede Zahl aus der nummerierten QUELLEN-LISTE oben stammt. Ohne Research-Quellen MUSS chart_config null sein.
- Nutze Chart.js-Format: { "type": "bar"|"line"|"radar"|"doughnut", "data": { "labels": [...], "datasets": [{ "label": "...", "data": [...] }] }, "title": "Diagramm-Titel", "unit": "%|€|Tage|Stunden|...", "source_ids": [1,2] }
- unit und source_ids sind PFLICHT. source_ids dürfen ausschließlich tatsächlich verwendete IDs aus der QUELLEN-LISTE enthalten.
...
- Beispiel: { "type": "bar", "data": { "labels": ["Bayern","NRW","Berlin"], "datasets": [{ "label": "Grunderwerbsteuer", "data": [3.5, 6.5, 6.0] }] }, "title": "Grunderwerbsteuer nach Bundesland", "unit": "%", "source_ids": [1] }
