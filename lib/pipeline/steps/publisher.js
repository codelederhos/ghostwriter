...
function normalizeGhostwriterHtml(content) {
  ...
  const absoluteAssets = String(content)
    .replace(/src=(")\/uploads\//g, `src=$1${base}/uploads/`)
    .replace(/src=(\')\/uploads\//g, `src=$1${base}/uploads/`);
  return normalizePublishedMediaHtml(absoluteAssets);
}
...
export async function syncPostToClient(postId) {
  const { rows: [row] } = await query(
    `SELECT p.*, ts.client_api_url, ts.client_api_key, ts.client_push_enabled, ts.review_flow
     FROM ghostwriter_posts p JOIN tenant_settings ts ON ts.tenant_id = p.tenant_id
     WHERE p.id = $1`,
  ...
  if (!row) return { synced: false, reason: "post_not_found" };
  if (!(row.client_push_enabled && row.client_api_url)) {
    return { synced: false, reason: "client_push_disabled" };
  }
  if (row.status === "draft_review") {
    const synced = await syncReviewDraftToClient(postId);
    return { synced, event: "post_review_draft" };
  }
  if (row.status !== "published") {
    return { synced: false, reason: `unsupported_status:${row.status}` };
  }
  if ((row.review_flow || "core") === "client") {
    return { synced: false, reason: "client_owns_published_record" };
  }
  let apiKey = row.client_api_key;
  if (apiKey) try { apiKey = decrypt(apiKey); } catch { /* schon Klartext */ }
  await pushToClientApi(
    { client_api_url: row.client_api_url, client_api_key: apiKey },
    row,
    row.blog_url
  );
  return { synced: true, event: "post_published" };
}
