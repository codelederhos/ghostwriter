...
export async function runPlanner(topics, language, override = null, tenantId = null) {
  ...
  // Override: forcedTitle (Quick-Generate) — direkter Titel-Override, ueberspringt Kategorie/Angle-Rotation
  if (override && override.forcedTitle) {
    const fallbackTopic = activeTopics[0] || { label: "Allgemein", description: "", default_cta: null };
    return {
      category: fallbackTopic.label,
      categoryLabel: fallbackTopic.label,
      categoryDesc: fallbackTopic.description || "",
      defaultCta: fallbackTopic.default_cta || null,
      angle: "Custom",
      angleName: "Manuell vorgegeben",
      angleDesc: "Vom Admin vorgegebener Artikel",
      season: season.id,
      seasonDesc: season.desc,
      language,
      forcedTitle: override.forcedTitle,
      forcedKeyword: override.forcedKeyword || null,
      briefing: override.briefing || null,
    };
  }
