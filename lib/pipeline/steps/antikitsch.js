...
// Gedankenstriche kontextsensitiv ersetzen: Zahlbereiche -> "bis",
// Satz-Einschübe -> Komma. NIE stumpf ": " (zerstört Sätze: "3—5" wurde "3: 5").
// En-Dash (–) nur mit Leerzeichen drumherum anfassen — Wort-Kopplungen bleiben.
function replaceDashesInHtml(html) {
  return String(html || "")
    .replace(/(\d[\d.,]*)\s*[—–]\s*(?=\d)/g, "$1 bis ")
    .replace(/\s+[—–]\s+/g, ", ")
    .replace(/\s*—\s*/g, ", ");
}
...
${bodyHtml}`;
  ...
  try {
    ...
    result = replaceDashesInHtml(result);
    // Trunkierungs-Schutz: Der Voll-Rewrite kann am 8192-Token-Limit abreißen —
    // deutlich kürzeres Ergebnis heißt fehlendes Artikel-Ende → Original behalten.
    if (!result || result.length < bodyHtml.length * 0.7) return bodyHtml;
    return result;
  } catch {
    return bodyHtml;
  }
