...
function insertBeforeFinalH2(body, html) {
  ...
  if (sourcesIdx > 0) return value.slice(0, sourcesIdx) + "\n" + html + "\n" + value.slice(sourcesIdx);
  return value + "\n" + html;
}
...
export function ensureRichArticle(article, { seo = {}, profile = {} } = {}) {
  ...
  const needsTable = !/<table\b/i.test(body);
  const needsProcess = !hasClass(body, "process-steps");
  const needsChecklist = !hasClass(body, "check-list");
  const needsCallout = !hasClass(body, "callout");
  const ctaCount = [...body.matchAll(/class=["'][^"']*(cta-block|cta-inline|callout--cta)[^"']*["']/gi)].length;
  const wordCount = stripHtml(body).split(/\s+/).filter(Boolean).length;

  if (needsTable || needsProcess || needsChecklist || needsCallout || ctaCount < 4 || wordCount < 950) {
    body = insertBeforeFinalH2(body, buildRichBlock({ keyword, companyName, ctaUrl }));
  }
