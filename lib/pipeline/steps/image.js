...
export async function runImageGen(settings, article, seo, plan = {}, tenantId = null, referenceImageUrls = [], opts = {}) {
  ...
  const styleFor = (scene, hasRefs) => hasRefs
    ? `${scene}. MANDATORY BRAND STYLE — regrade colors, lighting and mood of the result to strictly match: ${style}. Keep only the composition idea from the reference image(s); the brand palette always wins over the reference colors.`
    : `${scene}. ${style}`;
