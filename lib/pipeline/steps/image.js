...
export async function runImageGen(settings, article, seo, plan = {}, tenantId = null, referenceImageUrls = [], opts = {}) {
  ...
  const style = settings.image_style_prefix || "Documentary photography, Nikon D850 50mm, overcast natural window light, Kodak Portra 400 grain, slightly imperfect framing, worn surfaces, real-world imperfections, no faces, no text, no logos, no CGI, no artificial glow, no symmetry";
  ...
  let img1Source = refSelection.img1 ? "reference" : (img1?._skipped ? "none" : (webRef1 ? "web-reference" : "generated"));

  let img2 = img2Result.status === "fulfilled" ? img2Result.value : { ...img1 };
  let img2Source = img2Result.status === "rejected"
    ? img1Source
    : refSelection.img2 ? "reference" : (img2?._skipped ? "none" : (webRef2 ? "web-reference" : "generated"));
