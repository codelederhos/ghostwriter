...
export async function runImageGen(settings, article, seo, plan = {}, tenantId = null, referenceImageUrls = []) {
  ...
  if (refSelection.img1) {
    img1 = { url: refSelection.img1.image_url, localPath: refSelection.img1.image_url };
    ...
    img1 = await generateImage(settings, prompt1, article.slug, {
      format: format1,
      referenceImageUrl: webRef1,
    });
    if (webRef1) img1Source = "web-reference";
  }
  ...
  try {
    if (refSelection.img2) {
      img2 = { url: refSelection.img2.image_url, localPath: refSelection.img2.image_url };
      ...
      img2 = await generateImage(settings, prompt2, `${article.slug}-2`, {
        format: format2,
        referenceImageUrl: webRef2,
      });
      if (webRef2) img2Source = "web-reference";
    }
  } catch {
    img2 = { ...img1 };
