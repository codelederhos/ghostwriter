...
export async function runImageGen(settings, article, seo) {
  const prompt1 = buildImagePrompt(article, seo, settings.image_style_prefix, "title");
  const img1 = await generateImage(settings, prompt1, article.slug);

  let img2 = { url: null, localPath: null };
  try {
    const prompt2 = buildImagePrompt(article, seo, settings.image_style_prefix, "article");
