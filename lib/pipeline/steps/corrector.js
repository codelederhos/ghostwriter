...
/**
 * Position des letzten Content-<h2> (= Fazit) — ignoriert die <h2> der
 * Quellen-Section und des AEO-About-Blocks am Artikelende.
 */
function lastContentH2(html) {
...
export async function runCorrector(article, seo, profile, issues, settings) {
  ...
  if (article.gbp_text?.length > 1500) {
    // Am Satzende kürzen, nie mitten im Wort (GBP-Hard-Limit 1500)
    const cut = article.gbp_text.slice(0, 1497);
    const lastSentence = Math.max(cut.lastIndexOf(". "), cut.lastIndexOf("! "), cut.lastIndexOf("? "));
    fixes.gbp_text = lastSentence > 600 ? cut.slice(0, lastSentence + 1) : cut + "...";
  }
...
Antworte NUR mit dem HTML-Inhalt des neuen Abschnitts — kein JSON, nur HTML.`;
        ...
        // Vor dem letzten Content-<h2> (Fazit) einfügen — Quellen/About-Block ignorieren
        const lastH2Pos = lastContentH2(article.body_html);
        if (lastH2Pos > 0) {
...
Antworte NUR als JSON:
{
  ...
  "extra_block": "HTML mit ALLEN fehlenden Pflicht-Bausteinen zum Einfügen vor dem letzten H2 — sonst null"
}

WICHTIG zu extra_block: Wenn die Probleme fehlende Bausteine nennen, erzeuge in extra_block GENAU die fehlenden Typen mit diesen CSS-Klassen und themenbezogenem Inhalt aus dem Artikel:
- Checkliste fehlt → <ul class="check-list"><li>…</li>…</ul> (4-6 Punkte, konkret zum Thema)
- Schritt-für-Schritt fehlt → <ol class="process-steps"><li><strong>Titel</strong> Ein Satz Subtext.</li>…</ol> (4-5 Schritte)
- Tabelle fehlt → <table class="comparison-table"><thead>…</thead><tbody>…</tbody></table> (3+ Zeilen)
Mehrere fehlende Typen = mehrere Blöcke hintereinander in extra_block. Inhalte MÜSSEN aus dem Artikelthema abgeleitet sein, nie generisch.`;
  ...
  try {
    ...
    // Auf evtl. schon vorhandenen Fixes aufsetzen (z.B. Wortanzahl-Expansion),
    // sonst überschreibt der Struktur-Pfad sie wieder (Audit 17.07.2026)
    const baseBody = fixes.body_html || article.body_html;
    let body = baseBody;
    ...
    // Zusätzlicher Content-Baustein: vor letztem Content-<h2> einfügen (Quellen/About ignorieren)
    if (parsed.extra_block && typeof parsed.extra_block === "string" && parsed.extra_block !== "null") {
      const lastH2Pos = lastContentH2(body);
      if (lastH2Pos > 0) {
        body = body.slice(0, lastH2Pos) + "\n" + parsed.extra_block + "\n" + body.slice(lastH2Pos);
      }
    }

    if (body !== baseBody) fixes.body_html = body;
  } catch {
