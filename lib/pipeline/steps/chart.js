...
export function buildChartEmbedHtml({
  ...
  const sourceLinks = sources
    .map((source) => {
      ...
      return safeUrl
        ? `<a href="${escapeHtml(safeUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(source.title || safeUrl)}</a>`
        : "";
    })
    .filter(Boolean)
    .join(", ");
  ...
  const safeConfig = String(configJson || "{}")
    .replace(/&/g, "\\u0026")
    .replace(/</g, "\\u003c")
    .replace(/>/g, "\\u003e")
    .replace(/\u2028/g, "\\u2028")
    .replace(/\u2029/g, "\\u2029");
  return `<figure class="gw-chart">${titleHtml}<script type="application/json" class="gw-chart-config">${safeConfig}<\/script>${metaHtml}</figure>`;
}
