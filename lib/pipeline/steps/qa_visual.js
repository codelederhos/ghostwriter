...
async function saveScreenshot(buffer, postId, viewportName) {
  await fs.mkdir(SCREENSHOT_DIR, { recursive: true });
  const filename = `post-${postId}-${viewportName}-${Date.now()}.png`;
  await fs.writeFile(path.join(SCREENSHOT_DIR, filename), buffer);
  const base = process.env.NEXT_PUBLIC_BASE_URL || "";
  return `${base}/uploads/qa/${filename}`;
}

async function prepareForVision(buffer) {
  return sharp(buffer)
    .resize({ width: 1500, height: 7000, fit: "inside", withoutEnlargement: true })
...
async function notifyTelegram(settings, postId, tenant, blogUrl, findings, screenshotUrl) {
  ...
  const text = [
    ...
    issues ? `\nIssues:\n${issues}` : null,
    screenshotUrl ? `\nDesktop-Screenshot: ${screenshotUrl}` : null,
  ].filter(Boolean).join("\n");
  try {
...
export async function runQaVisual({ tenant, settings, post, blogUrl }) {
  ...
  try {
    ...
    for (const audit of audits) {
      screenshotUrls[audit.viewport.name] = await saveScreenshot(audit.screenshot, post.id, audit.viewport.name);
    }

    const visionImages = await Promise.all(audits.map(async (audit) => ({
      label: `${audit.viewport.name} ${audit.viewport.width}x${audit.viewport.height}, vollständige Seite`,
      buffer: await prepareForVision(audit.screenshot),
      mediaType: "image/png",
    })));
    ...
    await query(
      ...
      [JSON.stringify(findings), screenshotUrls.desktop, post.id]
    );

    if (!findings.ok) {
      await notifyTelegram(settings, post.id, tenant, blogUrl, findings, screenshotUrls.desktop);
    }
    return { ok: findings.ok, executed: true, findings, screenshotUrl: screenshotUrls.desktop };
  } catch (error) {
