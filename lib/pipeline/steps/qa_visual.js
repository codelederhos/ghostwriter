...
const CHECK_PROMPT = `Du bist visueller QA-Reviewer für einen Unternehmensblog.
Du siehst dieselbe vollständige Artikelansicht auf Desktop, Mobile 390 px und Mobile 360 px.
...
Antworte ausschließlich mit gültigem JSON:
{
  ...
  "issues": [
    {
      "viewport": "desktop|mobile|mobile_360|both",
...
const BROWSER_AUDIT_FUNCTION = `export default async ({ page, context }) => {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const describe = (element) => {
      ...
      const ratioDelta = !["cover", "contain"].includes(objectFit) && naturalRatio > 0 && renderedRatio > 0
    ...
    const chartContainers = [...articleRoot.querySelectorAll(".gw-chart")];
    ...
    return {
      ...
      emptyChartCount: chartContainers.filter((element) => !element.querySelector("img,canvas")).length,
      brokenImages: images.filter((image) => image.naturalWidth === 0).map((image) => image.src),
    };
  });
  const screenshot = await page.screenshot({ type: "png", fullPage: true });
  const screenshotBase64 = typeof Buffer !== "undefined"
    ? Buffer.from(screenshot).toString("base64")
    : btoa(Array.from(screenshot, (byte) => String.fromCharCode(byte)).join(""));
  return {
    data: { metrics, screenshot: screenshotBase64 },
...
async function runBrowserAudit(url, viewport) {
  ...
  const screenshotValue = String(result.screenshot);
  const screenshot = /^\d+(?:,\d+)+$/.test(screenshotValue)
    ? Buffer.from(screenshotValue.split(",").map((value) => Number(value)))
    : Buffer.from(screenshotValue.replace(/^data:image\/[a-z0-9.+-]+;base64,/i, ""), "base64");
  return {
    viewport,
...
export async function prepareVisionImages(buffer, viewport) {
  ...
  const overview = await sharp(buffer)
    ...
    .resize({ width: 1050, height: 5600, fit: "inside", withoutEnlargement: false })
  ...
  // 360 px wird deterministisch vollständig vermessen. Für die Vision reicht
  // dort die Übersicht, weil die fast identische 390-px-Ansicht bereits mit
  // lesbaren Detailkacheln abgedeckt ist. Das hält den API-Request klar unter
  // dem Anthropic-Payloadlimit.
  if (viewport.name === "mobile_360") return images;

  const isMobile = viewport.width <= 420;
  const tileHeight = isMobile ? 1400 : 1800;
  const tileCount = Math.min(3, Math.max(1, Math.ceil(height / tileHeight)));
  ...
  for (const [index, top] of starts.entries()) {
    ...
    const tile = await sharp(buffer)
      ...
      .flatten({ background: "#ffffff" })
      .resize({ width: isMobile ? 1170 : 1320, withoutEnlargement: false })
...
function structuralIssues(audits) {
  ...
  for (const audit of audits) {
    ...
    if (Number(metrics.emptyChartCount || 0) > 0) {
      issues.push({
        viewport,
        area: "chart",
        severity: "high",
        description: `${metrics.emptyChartCount} Diagramm(e) enthalten weder Bild noch Canvas und sind unsichtbar.`,
      });
    }
...
export async function runQaVisual({
  tenant,
  settings,
  post,
  blogUrl,
  ...
  try {
    ...
    const audits = await Promise.all(QA_VIEWPORTS.map((viewport) => runBrowserAudit(blogUrl, viewport)));
    ...
    if (!findings.ok && notifyOnFailure) {
