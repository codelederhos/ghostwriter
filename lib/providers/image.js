 ...
 */
async function generateImgStudio(prompt, format, settings) {
  ...
  const model = settings.image_model && !/gpt|dall/i.test(settings.image_model) ? settings.image_model : "krea2";
  ...
  const startRes = await fetch(`${base}/api/generate/batch`, {
    ...
    body: JSON.stringify({ prompt, models: [model], width, height, count: 1, batchLabel: "gw-pipeline" }),
