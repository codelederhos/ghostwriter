...
// Harte Obergrenze pro LLM-Call: Ein hängender Socket ohne Timeout ließ sonst
// den ganzen Scheduler-Tick hängen — der Tenant-Tag fiel still aus, weil der
// Claim next_run_at schon fortgeschrieben hatte (Audit 17.07.2026).
...
export async function generateText(settings, systemPrompt, userPrompt) {
  ...
  // _model_override = expliziter Laufzeit-Override (z.B. Translator → Haiku).
  // settings.text_model war hier toter Code: getRecommendedModel liefert immer
  // einen Wert, das ||-Fallback griff nie (Audit 17.07.2026).
  const model = settings._model_override || await getRecommendedModel(provider) || settings.text_model;
