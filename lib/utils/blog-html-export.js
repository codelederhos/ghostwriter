...
/* Chart Container */
.gw-chart { width: min(100%, 860px); max-width: 100%; margin: 2.75rem auto; border-radius: 16px; border: 1px solid #e5e7eb; padding: 1.5rem; background: #fff; box-shadow: 0 16px 42px rgba(17,24,39,.08); overflow: hidden; }
.gw-chart canvas { display: block; width: 100%; max-width: 100%; max-height: 480px; }
.gw-chart script { display: none; }
.gw-chart__title { font-weight: 700; font-size: 1.1rem; line-height: 1.35; margin: 0 0 1rem; color: #111827; }
.gw-chart img { display: block; width: 100%; max-width: 100%; height: auto; object-fit: contain; border-radius: 10px; }
.gw-chart__meta { margin: .85rem 0 0; font-size: .75rem; line-height: 1.45; color: #6b7280; }
.gw-chart__meta a { color: inherit; text-decoration: underline; text-underline-offset: 2px; }
@media (max-width: 640px) { .gw-chart { margin: 2rem 0; padding: 1rem .75rem; border-radius: 12px; box-shadow: none; } }
/* Standalone Button-CTA */
.cta-button { display: inline-block; padding: 0.75rem 1.5rem; border-radius: 0.75rem; font-weight: 600; font-size: 0.9rem; color: #fff !important; text-decoration: none !important; background: linear-gradient(135deg, #4f46e5, #7c3aed); box-shadow: 0 3px 12px rgba(79,70,229,.35); transition: transform .15s ease, box-shadow .15s ease; }
.cta-button:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(79,70,229,.4); }
/* "Mehr zum Thema"-Related-Box */
.related-posts { margin: 2rem 0; padding: 1rem 1.25rem; border: 1px solid #e5e7eb; border-left: 3px solid #4f46e5; border-radius: 10px; background: #fafafa; }
.related-posts p { margin: 0 0 0.5rem; }
.related-posts ul { margin: 0; padding-left: 1.25rem; }
.related-posts li { margin: 0.3rem 0; }
/* CTA Block */
...
// ── 7. Chart.js ──────────────────────────────────────────────────────────────
if(document.querySelector('.gw-chart')){
  function initCharts(){
    document.querySelectorAll('.gw-chart').forEach(function(el){
      var cfg=el.querySelector('.gw-chart-config');
      if(!cfg||el.querySelector('canvas'))return;
      try{
        var config=JSON.parse(cfg.textContent);
        var P=['#4f46e5','#16a34a','#dc2626','#d97706','#0891b2','#7c3aed'];
        (config.data.datasets||[]).forEach(function(ds,i){
          var c=P[i%P.length];
          if(!ds.backgroundColor)ds.backgroundColor=['bar','horizontalBar'].indexOf(config.type)>=0?c+'cc':c+'22';
          if(!ds.borderColor&&config.type==='line'){ds.borderColor=c;ds.borderWidth=ds.borderWidth||2;ds.tension=0.35;}
        });
  ...
  else{var cs=document.createElement('script');cs.src='https://cdn.jsdelivr.net/npm/chart.js@4/dist/chart.umd.min.js';cs.onload=initCharts;document.head.appendChild(cs);}
}
