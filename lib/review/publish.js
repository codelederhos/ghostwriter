...
function deterministicToken(prefix, postId) {
  // Ohne Key wären pub-/adm-Tokens von jedem berechenbar — hart failen.
  if (!process.env.ENCRYPTION_KEY) throw new Error("ENCRYPTION_KEY fehlt — Review-Tokens nicht ableitbar");
  const mac = crypto
...
function findPostByDeterministicToken(kind, token) {
  const m = String(token).match(/^(adm|pub)-([0-9a-f-]{36})-([0-9a-f]{24})$/i);
  if (!m) return null;
  if (kind === "publish" && m[1] !== "pub") return null;
  if (kind === "preview" && m[1] !== "adm") return null;
  const expected = deterministicToken(kind, m[2]);
...
export async function findPostByReviewToken(kind, token) {
  ...
  const deterministicPostId = findPostByDeterministicToken(kind, token);
